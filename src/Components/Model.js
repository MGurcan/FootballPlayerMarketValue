...
const Model = () => {
  ...
  const [inputData, setInputData] = useState([
    [0, 0, 0, 0, 0, 0, 0, 2020, 10000000, 0, 0], // Replace these initial values with your desired default values
  ]);
  ...
  const handleChangeModel = () => {
    ...
    if (modelNo === 1) {
      setInputData([[0, 0, 0, 0, 0, 0, 0, 2020, 0, 0, 0]])
    }
    else if (modelNo === 0) {
              ...
              (modelNo === 0 || (modelNo === 1 && columnIndex >= 35)) && <label key={columnIndex} className='flex-item'>
                ...
                {modelNo === 0 ? (
                  columnIndex === 10 ? (
                    ...
                    // Selection Input for columnIndex 8
                    <div className='selection-inputs'>
                    <select
                      value={value}
                      onChange={(e) => handleInputChange(rowIndex, columnIndex, parseInt(e.target.value))} // Seçilen değeri parseInt ile sayıya dönüştürün
                    >
                    ...
                    // Numeric Input for other columns
                    <input
                      type="number"
                      value={value}
                  ...
                  // Numeric Input for other columns (when modelNo is not 0)
                  <input
                    type="number"
                    value={value}
                    onChange={(e) => handleInputChange(rowIndex, columnIndex, e.target.value)}
